...
const changeColor = (newPrimaryColor, theme, darkClass = 'support-theme-dark') => {
	return new Promise((resolve) => {
		const themeEleId = 'support-theme-var'
		...
		if (newPrimaryColor) {
			...
			document.body.removeAttribute('support-theme')
		}
		if (isRealDark) {
...
const getLocalSetting = (loadTheme) => {
	let localSetting = {}
	try {
		const theme = tool.data.get('SUPPORT_THEME')
		const themeColor = tool.data.get('SUPPORT_THEME_COLOR')
		localSetting = {
			theme,
			themeColor
		}
